...
def calculate(list):
 if len(list) == 9:
    ...
    'variance': [np.var(m, axis=0).tolist(), 
    np.var(m, axis=1).tolist(),
    np.var(m).tolist()],

    'standard deviation': [np.std(m, axis=0).tolist(), 
    np.std(m, axis=1).tolist(),
    np.std(m).tolist()],

    'max': [np.max(m, axis=0).tolist(), 
    np.max(m, axis=1).tolist(),
    np.max(m).tolist()],

    'min': [np.min(m, axis=0).tolist(), 
    np.min(m, axis=1).tolist(),
    np.min(m).tolist()],

    'sum': [np.sum(m, axis=0).tolist(), 
